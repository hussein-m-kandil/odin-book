...
describe('PostList', () => {
  ...
  it('should set the "following" parameter on posts', async () => {
    ...
    expect((postsMock.setParams.mock.calls[0][0] as HttpParams).get('following')).toBe('true');
    vi.clearAllMocks();
    await rerender({ partialUpdate: true, inputs: { authorId: posts[0].author.id } });
    expect(postsMock.load).toHaveBeenCalledTimes(1);
    expect(postsMock.reset).toHaveBeenCalledTimes(1);
    expect(postsMock.setParams).toHaveBeenCalledTimes(1);
    expect((postsMock.setParams.mock.calls[0][0] as HttpParams).get('following')).toBeNull();
    vi.clearAllMocks();
  });

  it('should set the "author" parameter on posts', async () => {
    const authorId = posts[0].id;
    const { rerender } = await renderComponent({ inputs: { authorId } });
    expect(postsMock.load).toHaveBeenCalledTimes(1);
    expect(postsMock.reset).toHaveBeenCalledTimes(1);
    expect(postsMock.setParams).toHaveBeenCalledTimes(1);
    expect(postsMock.setParams.mock.calls[0][0]).toBeInstanceOf(HttpParams);
    expect((postsMock.setParams.mock.calls[0][0] as HttpParams).get('author')).toBe(authorId);
    vi.clearAllMocks();
    await rerender({ partialUpdate: true, inputs: { authorId: '' } });
    expect(postsMock.load).toHaveBeenCalledTimes(1);
    expect(postsMock.reset).toHaveBeenCalledTimes(1);
    expect(postsMock.setParams).toHaveBeenCalledTimes(0);
    vi.clearAllMocks();
    await rerender({ partialUpdate: false });
    expect(postsMock.load).toHaveBeenCalledTimes(1);
    expect(postsMock.reset).toHaveBeenCalledTimes(1);
    expect(postsMock.setParams).toHaveBeenCalledTimes(0);
    vi.clearAllMocks();
    await rerender({ partialUpdate: true, inputs: { following: true } });
    expect(postsMock.load).toHaveBeenCalledTimes(1);
    expect(postsMock.reset).toHaveBeenCalledTimes(1);
    expect(postsMock.setParams).toHaveBeenCalledTimes(1);
    expect((postsMock.setParams.mock.calls[0][0] as HttpParams).get('author')).toBeNull();
    vi.clearAllMocks();
  });

  it('should prefer the "author" parameter over the "following"', async () => {
    const authorId = posts[0].id;
    await renderComponent({ inputs: { authorId, following: true } });
    expect(postsMock.load).toHaveBeenCalledTimes(1);
    expect(postsMock.reset).toHaveBeenCalledTimes(1);
    expect(postsMock.setParams).toHaveBeenCalledTimes(1);
    expect(postsMock.setParams.mock.calls[0][0]).toBeInstanceOf(HttpParams);
